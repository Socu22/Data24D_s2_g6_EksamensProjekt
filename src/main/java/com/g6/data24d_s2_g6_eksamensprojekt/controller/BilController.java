...
public class BilController {
    ...
    @Autowired BilTypeRepository  bilTypeRepository;
    @Autowired LagerRepository    lagerRepository;
    ...
    public String visBiler(HttpServletRequest request, Model model)
    {
        ...
        List<Bil> biler = bilRepository.hentEksisteredeBiler(); //selve bilerne der bliver vist
    ...
    // RequestMapping til at få vist data om en specifik bil
    @GetMapping("/VisBil")
    public String visBil(HttpServletRequest request, Model model){
        HttpSession session = faaSession(request, model);// Hvem der har Rettighed til at bruge metoden.
        if(session == null) return "redirect:/Logind";

        // hent vognNummer fra request url, og find associeret
        // bil, notationer, og potentiel lejeaftale fra databasen derudfra
        String vognNummer         = request.getParameter("vognNummer");
        ...
        double notationsPris = 0, skadePris = 0; // til sammenregning af samlet pris for hhv notationer/skader på bilen

        if(aftale != null) // hvis der er en lejeaftale associeret med bilen
        {
            // sæt "aktiv data" i session så det kan hentes af andre controllere, uden at skulle i url'en
            aftale.setBil(bil);
            session.setAttribute("aftaleId",aftale.getAftale_Id());
            session.setAttribute("lejeAftale",aftale);

            // hent potentielle skader associeret til aftalen og kom i modellen for display i client
            List<Notation> skader = notationRepository.hentSkader(aftale.getAftale_Id());
            model.addAttribute("skader", skader);

            // beregn summen af potentielle skader til beregning af aftales slutpris
            for (Notation skade : skader) {skadePris += skade.getPris();}
            model.addAttribute("skadePris", skadePris);
        }

        // beregn summen af potentielle fejl&mangler til endelig beregning af købspris
        for (Notation notation : notationer) {notationsPris += notation.getPris();}

        // sætter session attributter, som muligvis bruges efter omdirigering
        session.setAttribute("bil", bil);
        session.setAttribute("vognNummer", bil.getVognNummer());

        // sæt data i modellen til thymeleaf for at "bygger" html'en til client
    ...
    // Denne RequestMapping sætter en bils status, eller registrer an aftale som betalt
    ...
    public String opdaterBil(HttpServletRequest request, Model model)
    {
        ...
        // hent 'status' fra request og hhv "aktivt" vognNummer/aftaleId fra aktiv session
        String status     = request.getParameter("status");
        String vognNummer = (String) session.getAttribute("vognNummer");
        int aftaleId      = (int) session.getAttribute("aftaleId");

        // registrer betaling/sæt status i databasen
        if (status.equals("BETALT"))
            {aftaleRepository.saetBetalt(aftaleId);}
        else if (Bil.Status.getNames().contains(status)) // check om 'status' er en gyldig 'Bil.Status'
            {bilRepository.saetStatus(vognNummer, status);}

        return "redirect:/VisBil?vognNummer=" + vognNummer; // diriger tilbage til visning af den associerede bil
    }
